...
|---------|---------|---------|
| `/build-step` | Execute a numbered prompt | `/build-step 03` |
| `/frontend-design` | Build UI with the design system | `/frontend-design Add a settings page` |
| `/backend-api` | Build Rust/Axum endpoints | `/backend-api Add budget tracking` |
| `/verify` | Run verification checks | `/verify 06` |
| `/fix` | Debug and fix issues | `/fix TypeScript error in dashboard` |
...
```
/frontend-design Add a recurring transactions detector that highlights subscriptions
```

```
/frontend-design Add a month-over-month comparison card to the dashboard
```

```
/frontend-design Build a settings page with data export and theme toggle
```
...
```
/backend-api Add a GET /api/stats/recurring endpoint that detects recurring merchants
```

```
/backend-api Add a budget system with POST/GET /api/budgets endpoints
```
...
```
/fix TypeScript error: Property 'monthly' does not exist on type 'SummaryStats'
```

```
/fix Backend panics on CSV import with "column count mismatch"
```

```
/fix Sidebar active state not updating when navigating to /analytics
```
...
```
/verify frontend    # TypeScript compilation + dev server
/verify backend     # Docker build + health check + API smoke test
/verify all         # Both
/verify 06          # Run the verification section from prompt 06
```
